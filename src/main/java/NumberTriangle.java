...
public class NumberTriangle {
    ...
    public static NumberTriangle loadTriangle(String fname) throws IOException {
        ...
        int father = Integer.parseInt(br.readLine());
        NumberTriangle top = new NumberTriangle(father);
        NumberTriangle[] pre = new NumberTriangle[]{top};
        ...
        while (line != null) {
            ...
            NumberTriangle[] cur = new NumberTriangle[numbers.length];

            for (int i = 0; i < numbers.length; i++) {
                cur[i] = new NumberTriangle(Integer.parseInt(numbers[i]));
            }

            for (int i = 0; i < pre.length; i++) {
                pre[i].setLeft(cur[i]);
                pre[i].setRight(cur[i + 1]);
                }
