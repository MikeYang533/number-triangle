...
public class NumberTriangle {
    ...
    public static NumberTriangle loadTriangle(String fname) throws IOException {
        ...
        while (line != null) {
            ...
            NumberTriangle[] nums = new NumberTriangle[parts.length];
            for (int i = 0; i < parts.length; i++) {
                int value = Integer.parseInt(parts[i]);
                nums[i] = new NumberTriangle(value);
            }
            rows.add(nums);
            //read the next line
            line = br.readLine();
        ...
        for (int i = rows.size()-2; i >= 0; i--) {
            NumberTriangle[] currRow = rows.get(i);
            NumberTriangle[] nextRow = rows.get(i+1);
            for (int j = 0; j < currRow.length; j++) {
                currRow[j].left = nextRow[j];
                currRow[j].right = nextRow[j+1];
            }
        }
        top = rows.get(0)[0];
        return top;
