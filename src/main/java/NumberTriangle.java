...
public class NumberTriangle {
    ...
    public int retrieve(String path) {
        ...
        for (int i = 0; i < path.length(); i++) {
            ...
            if (ch == 'l') {
                if (cur.left == null) {
                    throw new IndexOutOfBoundsException("Path goes beyond left leaf at step " + i);
                }
                cur = cur.left;
            } else if (ch == 'r') {
                if (cur.right == null) {
                    throw new IndexOutOfBoundsException("Path goes beyond right leaf at step " + i);
                }
                cur = cur.right;
            } else {
                throw new IllegalArgumentException("Path contains invalid char: " + ch);
            }
        }
        return cur.getRoot();
    }
